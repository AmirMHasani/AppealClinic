...
import { findUserByEmail, getMembershipForUser, migrateUserPasswordHash } from "@/lib/db";
...
export async function getSession(): Promise<SessionUser | null> {
  ...
  try {
    ...
    const clinicId = payload.clinicId ? String(payload.clinicId) : "";
    const role = (payload.role as MembershipRole) || "coordinator";
    if (!clinicId) {
      // Legacy session without clinicId — fail closed; force re-login
      return null;
    }
    return {
      ...
      clinicId,
      role: role === "owner" ? "owner" : "coordinator",
      clinicName: payload.clinicName ? String(payload.clinicName) : undefined,
    };
  } catch {
    return null;
  }
...
/** Owner-only gate for destructive clinic ops. */
...
export async function verifyCredentials(
  ...
  if (!membership) {
    // Fail closed — no clinic membership
    return null;
  }
